...
def build_sheet_02_raw_inputs(wb: Workbook):
    ...
    Edge/vertex KPI values at O1/O2 refined across snapshot delivery cadence
    via Sheet 09 build (Bi-Directional layer per Lock #8.24 + #8.40 Snapshot
    Delivery Mode framing). O2/O3 face-KPI values are silent-zero placeholders
    until researcher normalization completes across subsequent snapshots.
    ...
    notes = [
        ...
        "Per Lock #8.9 + #8.36: 3 surviving edge-KPI promotions (BSC.L7 @ E2-10 O2 · BSC.C8 @ E5-8 O1 · BSC.I3 @ E10-12 O1) — values from Phase 2 CEN data; Sheet 09 §6.40 chain reads signatures from octave-kpi-spec.json (Snapshot Delivery Mode per §6.7). BSC.F4 reverted to F11 face per Lock #8.36.",
...
def build_sheet_09_bidirectional(wb: Workbook):
    ...
    #
    # §6.7 Honest Forward-Evolution + Snapshot Delivery Mode (Phase B.4 + 2026-05-26 reframe):
    #   - 5 HIGH-confidence (Sections 1-2 of source): E2-10, E5-8, E10-12, E7-11 (deprecated), V13 (deprecated)
    #   - 45 DRAFT-confidence (Sections 3-4 of source): researcher snapshot weights, refined across
    #     snapshot delivery cadence as CEN matures + Calibration Loop closes per Disclosure §6.7.
    #     NOT "pending CEN partnership-validation" — CEN at Phase 2 maturity provides INPUTS;
    #     Quannex authors the analytical layer; multi-snapshot delivery is the validation mechanism.
...
def build_sheet_13_diagnostics(wb: Workbook):
    ...
    for row_offset, octave_num in enumerate([1, 2, 3]):
        ...
        note_text = ("Sheet 04 cen_f<n>_o1_e_final consumer (Lock #8.35 canonical baseline)"
                     if octave_num == 1
                     else f"O{octave_num} TBD — Sheet 02 BSC KPI values refined across snapshot delivery cadence (next snapshot v1.2)")
...
def build_sheet_21_coherence_story(wb: Workbook):
    ...
    is_isnt_rows = [
        ...
        ("CEN BECOMING:", "→ Calibration Loop closed Phase 2 → Wk6-Wk8: prescribed actions REDUCED systemic tension. → Methodology validated for CEN at canonical κ=φ² baseline. → v1.0 SSOT ships as canonical math + narrative mirror. → v1.1 snapshot 2026-05-25 integrated 50 Bi-Directional signatures verbatim from source (5 HIGH researcher pre-validated + 45 DRAFT). → Next snapshot in a few weeks (v1.2): updated CEN inputs + Calibration Loop closures + researcher refinement of DRAFT signatures via Snapshot Delivery Mode (Disclosure §6 + §6.7). Future v1.x path also explored: add edge-KPI at E1-8 (Mode 5 carrier, ordinal E4) + O1 KPIs at F9 + F10 (close architectural blindness) + expand provenance to Sheet 15b Edge_Provenance (30 rows) + Sheet 15c Vertex_Provenance (20 rows) per original plan §7.A."),
        ...
        ("v1.1.0 PHASE 1c — Edge + Vertex KPI integration + §6.7 activation (Lock #8.40):", "Thesis-defense-blocker addressed: 'I can't use the mathematical output in the thesis while I don't have edge KPIs full namings and questions.' Source documents (3 cross-validated markdown files dated 2026-05-21/22 in POC/docs/cen-ssot/, totalling 2,197 lines) now integrated into SSOT as data layer + sheet exposure. (1) NEW data layer POC/companies/cen/octave-kpi-spec.json (Lock #8.40 single-source-of-truth chain): 30 canonical edges + 20 vertices + 1 deprecated E7-11 entry; each entry carries canonicalName + 3-octave questions verbatim + bscKpiPlacement with primaryOctave coordinate + signature 10/15-tuple from BiDirectional source + signatureConfidence (5 HIGH researcher pre-validated / 45 DRAFT researcher snapshot) + per-face rationale. (2) Sheet 07 Edges extended cols K-Q: Canonical Name + BSC KPI + Octave + Confidence + Base + O1/O2/O3 Questions. (3) Sheet 08 Vertices extended cols O-U: Canonical Name + Classification + BSC KPI + Confidence + O1/O2/O3 Questions. (4) Sheet 09 refactored to READ signatures from spec (replacing hardcoded HIGH + equal-weight 0.20 placeholders); new §6.7 Status column shows ACTIVATED (HIGH + active KPI) / ACTIVATED (HIGH; KPI deprecated #8.36) / DRAFT (researcher snapshot; refined across snapshot cadence via Calibration Loop closure). Methodological discovery surfaced (also recorded in Disclosure §6.7): octave-character of a KPI is INVARIANT of geometric placement — Lock #8.36 reversion moved BSC.F4 from E7-11 to F11 face but preserved O2 octave; same for BSC.L8 (V13 → F10 face, O2 preserved). Octave = measurement-property; topology = geometry-property; orthogonal. Verified: 23/23 sheets build clean · validator 0 issues including new Lock #8.40 cross-check (Sheets 07/08/09 ↔ octave-kpi-spec.json) · 51 signatures all sum to 1.0 mathematically. Self-audit corrected 2 octave attribution errors via W06v3 authoritative verification: BSC.C8 (E5-8) and BSC.I3 (E10-12) are both O1 (survival/existence), not O2 as initially assumed from Section 4 structural-codification framing."),
        ("v1.1 SNAPSHOT DELIVERY MODE — partnership-clarification 2026-05-26:", "Partnership-clarification surfaced a framing error in v1.1.0 §6.7 activation language ('PROPOSED — pending CEN partnership-validation, target Q3 2026'). Deimantas: 'CEN doesn't really have what to validate anything here, at least how I understand it — this is a Quannex deliverable based on their input. But the partnership with them is going to be an ongoing and as they operationally mature, this might definitely change. Right now we are delivering one snapshot, in a few weeks we will deliver another.' HONEST REFRAME: this SSOT is a Quannex deliverable, SNAPSHOT-in-time. v1.1 = snapshot 2026-05-25. Next snapshot in a few weeks (v1.2). Validation emerges across the SUCCESSION OF SNAPSHOTS via Calibration Loop closure (does prescribed action reduce systemic tension?) — NOT via one-time CEN ratification. Asymmetric partnership shape: Quannex authors analytical layer (methodology, interpretive choices, signature weights); CEN provides organizational reality (4-vector, BSC KPIs, organizational context); multi-snapshot delivery is the meeting place. CEN at Phase 2 maturity isn't asked to ratify per-signature interpretive weights — they provide the organizational reality each snapshot's prescriptions are tested against. Applied this turn: (a) Sheet 09 col N §6.7 Status text rewritten — 45 entries now read 'DRAFT (researcher snapshot; refined across snapshot cadence via Calibration Loop closure)' instead of stale 'PROPOSED — pending CEN partnership-validation'; (b) Sheet 09 Block A + Block D + footer all reflect Snapshot Delivery Mode framing; (c) Sheet 13 stale 'pending partnership-validation' text removed; (d) Disclosure §6.7 comprehensively rewritten with partnership-clarification quote + asymmetric-partnership table + Snapshot Delivery Mode subsection; (e) Disclosure §6 Calibration Loop gained 'Snapshot Delivery as Operational Mode' subsection naming v1.0/v1.1/v1.2 cadence; (f) README mirror updated. Trust-the-Geometry §6.6 applied to methodology LANGUAGE itself: when the language doesn't honor the actual partnership shape, fix the language not silently. The methodology's epistemic conservatism (researcher doesn't claim authority that doesn't exist) is preserved; the operational mechanism (snapshot delivery cadence) is clarified."),
    ]
